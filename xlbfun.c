...
NODE *args;
{
  ...
  if (symbolp(fun.n_ptr)) fun.n_ptr = xleval(fun.n_ptr);
...
NODE *expr;
{
  ...
  /* handle atoms */
  if (atom(expr)) val.n_ptr = expr;

  /* handle (comma <expr>) */
  else if (car(expr) == s_comma) {
    if (atom(cdr(expr))) xlfail("bad comma expression");
  ...
  /* handle ((comma-at <expr>) ... ) */
    ...
    if (atom(cdr(car(expr)))) xlfail("bad comma-at expression");
...
NODE *args;
{
  ...
  while (sym->n_symvalue == s_unbound) xlunbound(sym);
