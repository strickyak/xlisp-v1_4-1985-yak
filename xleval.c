...
NODE *fun, *args;
{
  ...
  if (fun == NIL) xlfail("bad function");

  /* evaluate the function */
  if (subrp(fun))
    val = (*fun->n_subr)(args);
  else if (consp(fun)) {
    if (car(fun) != s_lambda) xlfail("bad function type");
    val = evfun(fun, args);
  } else
    xlfail("bad function");
...
NODE *expr;
{
  ...
  if ((fun.n_ptr = xleval(fun.n_ptr)) == NIL) xlfail("bad function");

  /* evaluate the function */
  if (subrp(fun.n_ptr) || fsubrp(fun.n_ptr)) {
    if (subrp(fun.n_ptr)) args.n_ptr = xlevlist(args.n_ptr);
...
NODE *args;
{
  ...
  for (val = NIL; src.n_ptr; src.n_ptr = cdr(src.n_ptr)) {
    /* check this entry */
    if (!consp(src.n_ptr)) xlfail("bad argument list");
...
NODE *sym;
{
  ...
  if ((p = xlobsym(sym)) != NIL) return (car(p));

  /* get the value of the variable */
  while ((p = sym->n_symvalue) == s_unbound) xlunbound(sym);
...
NODE *fun, *args;
{
  ...
  if ((fun = cdr(fun)) == NIL || !consp(fun)) xlfail("bad function definition");

  /* get the formal argument list */
  if ((fargs = car(fun)) && !consp(fargs)) xlfail("bad formal argument list");
  ...
  /* execute the code */
  for (cptr.n_ptr = cdr(fun); cptr.n_ptr != NIL;) val = xlevarg(&cptr.n_ptr);
...
void xlabind(NODE *fargs, NODE *aargs) {
  ...
  if (consp(fargs) && car(fargs) == k_aux)
    while ((fargs = cdr(fargs)) != NIL && consp(fargs)) xlbind(car(fargs), NIL);
