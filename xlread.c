...
NODE *fptr, **pval;
{
  ...
  while (TRUE) /* check next character for type of node */
...
NODE *fptr;
{
  ...
  for (lastnptr = NIL; (ch = nextch(fptr)) != ')'; lastnptr = nptr) {
    ...
    if (ch == '.') {
      ...
      if (lastnptr == NIL) xlfail("invalid dotted pair");

      /* parse the expression after the dot */
      if (!parse(fptr, &p)) badeof(fptr);
      rplacd(lastnptr, p);

      /* make sure its followed by a close paren */
      if (nextch(fptr) != ')') xlfail("invalid dotted pair");
...
NODE *fptr;
{
  ...
  /* get symbol name */
  for (i = 0; i < STRMAX && issym(xlpeek(fptr));) sname[i++] = xlgetc(fptr);
  sname[i] = 0;
...
NODE *fptr;
{
  ...
  while ((ch = xlpeek(fptr)) != EOF && isspace(ch)) xlgetc(fptr);
  return (ch);
}
...
NODE *fptr;
{
  ...
  if ((ch = xlgetc(fptr)) == EOF) badeof(fptr);
  return (ch);
}
