...
NODE *args;
{
  ...
  /* read an expression */
  if (!xlread(fptr.n_ptr, &val)) val = eof.n_ptr;
...
int intflag;
{
  ...
  for (p = buf; consp(list); list = cdr(list)) {
    if ((val = car(list)) == NIL || !fixp(val)) xlfail("bad character list");
    if ((int)(p - buf) < STRMAX) *p++ = val->n_int;
  }
  *p = 0;
...
NODE *args;
{
  ...
  if (fptr->n_fp == NULL) xlfail("file not open");
...
NODE **pargs;
{
  ...
  if (arg = xlarg(pargs)) {
    if (filep(arg)) {
      if (arg->n_fp == NULL) xlfail("file not open");
    } else if (!consp(arg))
