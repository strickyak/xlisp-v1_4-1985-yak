...
NODE **pargs;
{
  ...
  if (!consp(*pargs)) xlfail("too few arguments");
...
NODE **pargs;
{
  ...
  /* check its type */
  if (type == LIST) {
    if (arg && ntype(arg) != LIST) xlfail("bad argument type");
  } else {
    if (arg == NIL || ntype(arg) != type) xlfail("bad argument type");
  }
...
void xltest(NODE **pfcn, int *ptresult, NODE **pargs) {
  ...
  *pargs = cdr(*pargs);
  ...
  /* make sure the argument exists */
  if (!consp(*pargs)) xlfail("no value for keyword argument");

  /* get the argument value */
  *pfcn = car(*pargs);

  /* if its a symbol, get its value */
  if (symbolp(*pfcn)) *pfcn = xleval(*pfcn);
...
void xllastarg(NODE *args) {
  if (args) xlfail("too many arguments");
}
