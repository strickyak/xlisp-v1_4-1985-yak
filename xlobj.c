...
NODE *obj, *msg, *args;
{
  ...
  if (!subrp(method.n_ptr) && !consp(method.n_ptr)) xlfail("bad method");
  ...
  if (subrp(method.n_ptr)) {
    ...
    while (cptr.n_ptr != NIL) val.n_ptr = xlevarg(&cptr.n_ptr);
  }
...
NODE *cls, *msg;
{
  ...
  for (lptr = car(ivar); lptr != NIL; lptr = cdr(lptr))
    if (car(mptr = car(lptr)) == msg) return (mptr);
...
int varnum;
{
  ...
  varcnt = 0;
  for (vptr = vars.n_ptr; consp(vptr); vptr = cdr(vptr)) {
    /* make sure this is a valid symbol in the list */
    if ((sym = car(vptr)) == NIL || !symbolp(sym)) xlfail("bad variable list");

    /* make sure its not already defined */
    if (checkvar(cls, sym)) xlfail("multiply defined variable");
  ...
  if (vptr != NIL) xlfail("bad variable list");
...
NODE *cls, *sym;
{
  ...
  while (msgcls != NIL) {
    ...
    for (lptr = xlgetivar(msgcls, MESSAGES); lptr != NIL; lptr = cdr(lptr))
      if ((msg = car(lptr)) != NIL && car(msg) == sym) return (msg);
...
NODE *obj, *sym;
{
  ...
  for (; cls != NIL; cls = xlgetivar(cls, SUPERCLASS)) {
    ...
    if (!found && cls == msgclass->n_symvalue) found = TRUE;

    /* lookup the instance variable */
    varnum = 0;
    for (lptr = xlgetivar(cls, IVARS); lptr != NIL; lptr = cdr(lptr))
    ...
    varnum = 0;
    for (lptr = xlgetivar(cls, CVARS); lptr != NIL; lptr = cdr(lptr))
      if (car(lptr) == sym)
...
NODE *xlgetivar(NODE *obj, int num) { return (car(xlivar(obj, num))); }
