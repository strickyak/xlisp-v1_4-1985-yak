...
NODE *args;
{
  ...
  while (arg.n_ptr) {
    ...
    if (xlevarg(&list.n_ptr)) {
      ...
      while (list.n_ptr) val = xlevarg(&list.n_ptr);
...
NODE *args;
{
  ...
  while (arg.n_ptr) /* get the next argument */
    if ((val = xlevarg(&arg.n_ptr)) == NIL) break;
...
NODE * xor (args) NODE *args;
{
  ...
  /* evaluate each argument */
  while (arg.n_ptr)
    if ((val = xlevarg(&arg.n_ptr))) break;
...
int pflag;
{
  ...
  /* execute the code */
  for (val = NIL; arg.n_ptr;) val = xlevarg(&arg.n_ptr);
...
int n;
{
  ...
  while (n--) val.n_ptr = xlevarg(&arg.n_ptr);

  /* evaluate each remaining argument */
  while (arg.n_ptr) xlevarg(&arg.n_ptr);
...
int pflag;
{
  ...
  if (!rbreak)
    for (rval = NIL; consp(clist.n_ptr);) rval = xlevarg(&clist.n_ptr);
...
NODE *args;
{
  ...
  /* check for 'throw' */
  if (setjmp(cntxt.c_jmpbuf)) val = xlvalue;

  /* otherwise, evaluate the remainder of the arguments */
  else {
    while (arg.n_ptr) val = xlevarg(&arg.n_ptr);
  }
  xlend(&cntxt);
...
NODE *args;
{
  ...
  /* check for error */
  if (setjmp(cntxt.c_jmpbuf)) val = NIL;
...
LOCAL int tagblock(NODE *code, NODE **pval) {
  ...
  /* otherwise, enter the body */
    ...
    /* check for a 'go' */
    if (type == CF_GO) arg.n_ptr = xlvalue;
