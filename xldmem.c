...
int type;
{
  ...
  if ((nnode = fnodes) == NIL) {
    gc();
    if ((nnode = fnodes) == NIL) xlabort("insufficient node space");
  }
...
int size;
{
  ...
  if ((sptr = malloc(size + 1)) == NULL) {
    gc();
    if ((sptr = malloc(size + 1)) == NULL) xlfail("insufficient string space");
  }
  total += (long)(size + 1);
  ...
  for (seg = segs; seg != NULL; seg = seg->sg_next) {
    ...
    for (n = seg->sg_size; n--; p++)
      if (!(p->n_flags & MARK)) {
        switch (ntype(p)) {
          case STR:
            if (p->n_strtype == DYNAMIC && p->n_str != NULL) strfree(p->n_str);
            break;
          case FPTR:
            if (p->n_fp) fclose(p->n_fp);
            break;
        }
        p->n_type = FREE;
        p->n_flags = 0;
