...
NODE *args;
{
  ...
  while (args) total += strlen(xlmatch(STR, &args)->n_str);
...
NODE *args;
{
  ...
  for (p = args, len = 0; p;) len += strlen(xlmatch(STR, &p)->n_str);
  ...
  *str = 0;
  ...
  /* combine the strings */
  while (args) strcat(str, xlmatch(STR, &args)->n_str);
...
NODE *args;
{
  ...
  if (start + forlen > srclen) forlen = srclen - start + 1;
