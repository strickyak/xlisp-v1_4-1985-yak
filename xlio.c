...
NODE *fptr;
{
  ...
  /* otherwise, check for input from a stream */
    ...
    if ((lptr = car(fptr)) == NIL)
      ...
      if (!consp(lptr) || (cptr = car(lptr)) == NIL || !fixp(cptr))
        xlfail("bad stream");
      if (rplaca(fptr, cdr(lptr)) == NIL) rplacd(fptr, NIL);
      ch = cptr->n_int;
    }
  }
  ...
  /* otherwise, get a new character */
    ...
    if (prompt && fp == stdin) {
      ...
      if (xldebug) printf("%d:", xldebug);

      /* print the nesting level */
      if (xlplevel > 0) printf("%d", xlplevel);
