...
void xlfail(char *emsg) { xlerror(emsg, stacktop()); }

/* xlabort - xlisp serious error handler */
void xlabort(char *emsg) { xlsignal(emsg, s_unbound); }
...
void xlerror(char *emsg, NODE *arg) { doerror(NULL, emsg, arg, FALSE); }
...
LOCAL void doerror(char *cmsg, char *emsg, NODE *arg, int cflag) {
  ...
  if (s_breakenable->n_symvalue == NIL) xlsignal(emsg, arg);
...
int cflag;
{
  ...
  if (expr.n_ptr == s_quit) xlsignal("quit from break loop", s_unbound);
}

/* tpush - add an entry to the trace stack */
void xltpush(NODE *nptr) {
  if (++xltrace < TDEPTH) trace_stack[xltrace] = nptr;
}

/* tpop - pop an entry from the trace stack */
void xltpop(void) { xltrace--; }
...
void xlbaktrace(int n) {
  ...
  for (i = xltrace; (n < 0 || n--) && i >= 0; i--)
    if (i < TDEPTH) stdprint(trace_stack[i]);
}
