...
char *adstr;
{
  ...
  if (*adstr && list) xlfail("bad argument");
...
NODE *args;
{
  ...
  /* find the last cons */
  while (consp(list) && cdr(list)) list = cdr(list);
...
int tresult;
{
  ...
  for (; consp(alist); alist = cdr(alist))
    if ((pair = car(alist)) && consp(pair))
      if (dotest(expr, car(pair), fcn) == tresult) return (pair);
  return (NIL);
}
...
int cdrflag;
{
  ...
  /* get n and the list */
  if ((n = xlmatch(INT, &args)->n_int) < 0) xlfail("bad argument");
  if ((list = xlmatch(LIST, &args)) == NIL) xlfail("bad argument");
  xllastarg(args);

  /* find the nth element */
  for (; n > 0 && consp(list); n--) list = cdr(list);
...
int carflag, valflag;
{
  ...
  if (!valflag) val.n_ptr = lists.n_ptr;
  ...
  if (symbolp(fcn.n_ptr)) fcn.n_ptr = xleval(fcn.n_ptr);
...
NODE *args;
{
  ...
  while (consp(list.n_ptr)) {
    if (dotest(x.n_ptr, car(list.n_ptr), fcn.n_ptr) != tresult) break;
    list.n_ptr = cdr(list.n_ptr);
  }
  val = last = list.n_ptr;
